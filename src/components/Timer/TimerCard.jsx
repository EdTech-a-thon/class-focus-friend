  ...
  return (
    <section className={`card timer-card ${focusMode ? "" : "timer-setup-card combined-session-setup"}`}>
      {focusMode
        ...
        : <>
            <div className="session-setup-fields"><SessionSettingsCard session={session} displayCountdown={displayCountdown}><NoiseCard noise={noise} embedded /></SessionSettingsCard></div>
          </>}
    </section>
  )
}
